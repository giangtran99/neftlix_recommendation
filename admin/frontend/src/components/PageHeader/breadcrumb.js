...
export default class BreadcrumbView extends PureComponent {
  ...
  conversionFromLocation = (routerLocation, breadcrumbNameMap) => {
    ...
    // console.log('BreadcrumbView -> conversionFromLocation -> pathSnippets %o home %o', pathSnippets, home);
    // Loop data mosaic routing
    const extraBreadcrumbItems = pathSnippets.map((url, index) => {
      const currentBreadcrumb = getBreadcrumb(breadcrumbNameMap, url);
      // console.log('BreadcrumbView -> conversionFromLocation -> currentBreadcrumb', currentBreadcrumb);
      if (currentBreadcrumb.inherited) {
        return null;
      }
      const isLinkable = index !== pathSnippets.length - 1 && currentBreadcrumb.component;
      const name = itemRender ? itemRender(currentBreadcrumb) : currentBreadcrumb.name;
      // console.log('BreadcrumbView -> conversionFromLocation -> itemRender %o, name %o', itemRender, name);
  ...
  conversionBreadcrumbList = () => {
    ...
    // console.log('BreadcrumbView -> conversionBreadcrumbList -> breadcrumbList %o \n breadcrumbSeparator %o \n', breadcrumbList, breadcrumbSeparator);
    const { routes, params, routerLocation, breadcrumbNameMap } = this.getBreadcrumbProps();
    // console.log('BreadcrumbView -> conversionBreadcrumbList -> routes %o \n, params %o \n, routerLocation %o \n, breadcrumbNameMap %o \n', routes, params, routerLocation, breadcrumbNameMap);
