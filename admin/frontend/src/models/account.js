...
export default {
  ...
  effects: {
    *fetch({ payload, callback }, { call, put }) {
			...
			let list = [];
			try{
				const response = yield call(accountServices.getList, payload);
				if (response) {
					let list = [];
					if(response[`rows`]) {
    ...
    *fetchForm({ payload, callback }, { call, put }) {
			let response;
			try {
				response = yield call(accountServices.get, payload.id, payload);
				if (response)
    ...
    *submit({ payload, callback }, { call, put, select }) {
      ...
      try {
        if (formType === 'E') {
          ...
          response = yield call(accountServices.update, id, params, false);
          if (response && response.id) {
            success = true;
          }
        } else {
          response = yield call(accountServices.create, params, false);
          if (response && response.id) {
            success = true;
          }
        }
      ...
      if (success) {
		...
		*find({ payload, callback }, { call, put }) {
			let list = [];
			try {
				const response = yield call(accountServices.find, payload);
				if (response) {
